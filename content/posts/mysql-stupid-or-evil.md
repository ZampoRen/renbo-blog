...
但视频作者展示的 MySQL 案例里，事情偏偏就这么发生了：因为订单状态和买家 ID 出现在同一个 composite index 里，更新订单状态时，父表也被卷进了锁行为。
...
真正反直觉的是另一种情况：假设业务为了查询方便，建了一个包含 `status` 和 `buyer_id` 的复合索引。视频作者展示的案例里，只更新 `status` 这种非外键字段，也会牵动父表锁。
